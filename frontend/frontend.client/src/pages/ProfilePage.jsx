...
const EducationCard = ({ profile }) => {
    ...
    const [form, setForm] = useState({ university: '', degree: '', major: '', gpa: '', year: '' });
    ...
    const submit = (e) => {
        ...
        setForm({ university: '', degree: '', major: '', gpa: '', year: '' });
        setEditId(null);
        setOpen(false);
    };

    const startEdit = (item) => {
        setForm({ university: item.university, degree: item.degree, major: item.major, gpa: item.gpa, year: item.year });
        setEditId(item.id);
        setOpen(true);
    };
    ...
    return (
        <Card icon={GraduationCap} title="Education" subtitle="Your academic background"
            action={
                <button className="pf-btn pf-btn--primary pf-btn--sm" onClick={() => { setEditId(null); setForm({ university: '', degree: '', major: '', gpa: '', year: '' }); setOpen(true); }}>
                    <Plus size={14} /> Add
                </button>
            }>
            {open && (
                <div className="pf-modal-overlay" onClick={() => setOpen(false)}>
                    <div className="pf-modal" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={submit} className="pf-modal__form">
                            <div className="pf-field">
                                <label className="pf-label">University</label>
                                <input className="pf-input" value={form.university} onChange={e => setForm(f => ({ ...f, university: e.target.value }))} required />
                            </div>
                            <div className="pf-form-grid">
                                <div className="pf-field">
                                    <label className="pf-label">Degree</label>
                                    <input className="pf-input" value={form.degree} onChange={e => setForm(f => ({ ...f, degree: e.target.value }))} placeholder="B.S., M.S." required />
                                </div>
                                <div className="pf-field">
                                    <label className="pf-label">Major</label>
                                    <input className="pf-input" value={form.major} onChange={e => setForm(f => ({ ...f, major: e.target.value }))} required />
                                </div>
                            </div>
                            <div className="pf-form-grid">
                                <div className="pf-field">
                                    <label className="pf-label">GPA (optional)</label>
                                    <input className="pf-input" value={form.gpa} onChange={e => setForm(f => ({ ...f, gpa: e.target.value }))} placeholder="3.8" />
                                </div>
                                <div className="pf-field">
                                    <label className="pf-label">Graduation Year</label>
                                    <input className="pf-input" value={form.year} onChange={e => setForm(f => ({ ...f, year: e.target.value }))} placeholder="2025" required />
                                </div>
                            </div>
                            <div className="pf-modal__footer">
                                <button type="button" className="pf-btn pf-btn--outline" onClick={() => setOpen(false)}>Cancel</button>
                                <button type="submit" className="pf-btn pf-btn--primary">{editId ? 'Update' : 'Add'}</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            {items.length === 0 ? (
                <div className="pf-empty">
                    <GraduationCap size={40} className="pf-empty__icon" />
                    ...
                    {items.map(item => (
                        <div key={item.id} className="pf-list-item">
                            <div className="pf-list-item__body">
                                ...
                                <p className="pf-list-item__meta">Class of {item.year}{item.gpa ? ` · GPA: ${item.gpa}` : ''}</p>
                            </div>
                            <div className="pf-list-item__actions">
                                <button className="pf-icon-btn" onClick={() => startEdit(item)}><Edit size={15} /></button>
                                <button className="pf-icon-btn pf-icon-btn--danger" onClick={() => setItems(prev => prev.filter(i => i.id !== item.id))}><Trash2 size={15} /></button>
                            </div>
                        </div>
                    ))}
                </div>
...
const SkillsCard = ({ profile }) => {
    const [skills, setSkills] = useState([
        ...
        { name: 'Python', proficiency: 'Intermediate', verified: false },
    ]);
    const [name, setName] = useState('');
    const [proficiency, setProficiency] = useState('Beginner');

    const add = () => {
        if (name.trim() && !skills.find(s => s.name === name.trim())) {
            setSkills(prev => [...prev, { name: name.trim(), proficiency, verified: false }]);
            setName('');
        }
    };
        ...
        <Card icon={Target} title="Skills" subtitle="Skills directly affect your match score"
            ...
            {skills.length > 0 ? (
                <div className="pf-tags">
                    {skills.map(s => (
                        <span key={s.name} className="pf-tag pf-tag--skill">
                            {s.verified && <CheckCircle size={11} className="pf-tag__verified" />}
                            {s.name}
                            <span className="pf-tag__level">({s.proficiency})</span>
                            <button className="pf-tag__remove" onClick={() => setSkills(prev => prev.filter(x => x.name !== s.name))}>
                                <X size={11} />
                            </button>
                        </span>
                    ))}
...
const InterestsCard = () => {
    const [interests, setInterests] = useState(['Artificial Intelligence', 'Web Development']);
        ...
        <Card icon={Heart} title="Interests" subtitle="Areas you're passionate about">
            ...
            {interests.length > 0 && (
                <div>
                    ...
                    <div className="pf-tags">
                        {interests.map(i => (
                            <span key={i} className="pf-tag pf-tag--interest">
                                {i}
                                <button className="pf-tag__remove" onClick={() => setInterests(prev => prev.filter(x => x !== i))}>
                                    <X size={11} />
                                </button>
                            </span>
                        ))}
                            ...
                            <div className="pf-timeline-content">
                                <div className="pf-list-item">
                                    ...
                                    <div className="pf-list-item__actions">
                                        ...
                                        <button className="pf-icon-btn pf-icon-btn--danger" onClick={() => setItems(prev => prev.filter(x => x.id !== exp.id))}><Trash2 size={15} /></button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
const ProjectsCard = () => {
    ...
    const submit = (e) => {
        ...
        if (editId) {
            setItems(prev => prev.map(i => i.id === editId ? { ...form, techStack, id: editId } : i));
        } else {
            setItems(prev => [...prev, { ...form, techStack, id: Date.now().toString() }]);
        }
                    ...
                    {items.map(proj => (
                        <div key={proj.id} className="pf-list-item">
                            ...
                            <div className="pf-list-item__actions">
                                ...
                                <button className="pf-icon-btn pf-icon-btn--danger" onClick={() => setItems(prev => prev.filter(x => x.id !== proj.id))}><Trash2 size={15} /></button>
                            </div>
                        </div>
                    ))}
                </div>
