...
export default function OrganizationDashboard() {
    ...
    async function fetchListings() {
        ...
        try {
            ...
            const mapped: Listing[] = data.map((o: any) => ({
                ...
                benefitsPerks: Array.isArray(o.benefits) ? o.benefits.join(", ") : o.benefitsPerks || "",
    ...
    async function fetchApplicationsForOpportunity(opportunityId: number) {
        try {
            ...
            const res = await api.get(`/application/recruiter/opportunity/${opportunityId}`);
            const data = res.data;
            ...
            setApplicationsError(err?.response?.data || err?.message || "Failed to fetch applications.");
        } finally {
    ...
    async function fetchApplicationDetails(applicationId: number) {
        try {
            ...
            const res = await api.get(`/application/recruiter/${applicationId}`);
            setSelectedApplication(res.data);
        } catch (err: any) {
            setApplicationDetailsError(err?.response?.data || err?.message || "Failed to fetch application details.");
        } finally {
    ...
    async function updateApplication(applicationId: number) {
        try {
            await api.patch(`/application/${applicationId}/recruiter`, {
            ...
            alert(err?.response?.data || err?.message || "Updating the application went wrong.");
        }
    }
    ...
    async function publish() {
        ...
        const payload = {
            ...
            level: formData.employmentType === "internship" ? "Intern" : "Entry",
            ...
            benefits: formData.benefitsPerks
                ? formData.benefitsPerks
                    .split("\n")
                    .map((x) => x.trim())
                    .filter(Boolean)
                : [],
        ...
        try {
            ...
            console.log("Created opportunity:", res.data);
    ...
    async function reopenJob(id: number) {
        try {
    ...
    const responsiveHeaderRow: React.CSSProperties = {
        ...styles.headerRow,
        flexDirection: isMobile ? "column" : "row",
        alignItems: isMobile ? "stretch" : "center",
    };

    const responsiveOrgLeft: React.CSSProperties = {
        ...styles.orgLeft,
        alignItems: isMobile ? "flex-start" : "center",
    };

    const responsiveAvatar: React.CSSProperties = {
        ...styles.orgAvatar,
    ...
    const responsiveTabs: React.CSSProperties = {
        ...styles.tabs,
        overflowX: "auto",
        whiteSpace: "nowrap",
    };

    const responsiveFormGrid3: React.CSSProperties = {
        ...styles.formGrid3,
        gridTemplateColumns: isMobile ? "1fr" : isTablet ? "1fr 1fr" : "1fr 1fr 1fr",
    };

    const responsiveFormGrid2: React.CSSProperties = {
        ...styles.formGrid2,
        gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr",
    };

    const responsiveListingTop: React.CSSProperties = {
        ...styles.listingTop,
        flexDirection: isMobile ? "column" : "row",
    ...
    const responsiveTopButtons: React.CSSProperties = {
        display: "flex",
        gap: 10,
        flexWrap: "wrap",
    ...
    const responsivePrimaryButton: React.CSSProperties = {
        ...styles.primaryBtn,
        width: isMobile ? "100%" : "auto",
    };

    const responsiveDangerButton: React.CSSProperties = {
        ...styles.dangerBtn,
        width: isMobile ? "100%" : "auto",
    };

    const responsiveSuccessButton: React.CSSProperties = {
        ...styles.successBtn,
        width: isMobile ? "100%" : "auto",
    };
    ...
    if (loadingProfile) {
        return (
            <div style={responsivePageStyle}>
                <div style={responsiveCardStyle}>Loading organization profile...</div>
            </div>
        );
    }
        ...
        return (
            <div style={responsivePageStyle}>
                <div style={responsiveCardStyle}>
                    <p style={{ ...styles.small, color: "#b91c1c" }}>{profileError}</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div style={responsivePageStyle}>
            <div style={responsiveCardStyle}>
                <div style={responsiveHeaderRow}>
                    <div style={responsiveOrgLeft}>
                        <div style={responsiveAvatar}>
                            {(companyName || "R").slice(0, 2).toUpperCase()}
                        </div>
                        <div>
                            <h1 style={styles.h1}>{companyName || "Recruiter Dashboard"}</h1>
                            <p style={styles.muted}>Post jobs • manage listings • close/reopen roles</p>
                        </div>
                    </div>

                    <div style={responsiveTopButtons}>
                        <button style={responsiveButton} type="button" onClick={() => setActiveTab("post")}>
                            Post a job
                        </button>
                        <button style={responsiveButton} type="button" onClick={() => setActiveTab("listings")}>
                            View listings
                        </button>
                    </div>
                </div>
                ...
                <div style={responsiveTabs}>
                    <button
                        type="button"
                        style={{ ...styles.tabBtn, ...(activeTab === "post" ? styles.tabBtnActive : {}) }}
                ...
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
                    <div style={responsiveCardStyle}>
                        ...
                        <div style={responsiveActions}>
                            <button type="button" style={responsivePrimaryButton} onClick={publish}>
                                Publish Job
                            </button>
                            <button type="button" style={responsiveButton} onClick={saveDraft}>
                                Save Draft
                            </button>
                        </div>
                    </div>
            ...
            {activeTab === "listings" && (
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
                    <div style={responsiveCardStyle}>
                        ...
                        {listingsError && <p style={{ ...styles.small, color: "#b91c1c" }}>{listingsError}</p>}
                        {!loadingListings && listings.length === 0 && !listingsError && (
                            <p style={styles.small}>No listings found for this recruiter yet.</p>
                        )}
                    </div>
                    ...
                    <div style={{ display: "grid", gap: 12 }}>
                        {listings.map((job) => (
                            <div key={job.id} style={styles.listingCard}>
                                <div style={responsiveListingTop}>
                                    <div>
                                    ...
                                    <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                                        ...
                                        {job.status === "active" && (
                                            <button type="button" style={responsiveDangerButton} onClick={() => closeJob(job.id)}>
                                                <XCircle size={16} /> Close
                                            </button>
                                        )}

                                        {job.status === "closed" && (
                                            <button type="button" style={responsiveSuccessButton} onClick={() => reopenJob(job.id)}>
                                ...
                                <div style={{ display: "flex", gap: 10, alignItems: "center", marginTop: 12, flexWrap: "wrap" }}>
                                    <span style={{ fontWeight: 700 }}>Applicants: {job.applicants}</span>

                                    <button
                                        type="button"
                                        style={responsiveButton}
                                        onClick={() => fetchApplicationsForOpportunity(job.id)}
                                    >
                                        View Applicants
                                    </button>
                                </div>
            ...
            {activeTab === "applications" && (
                <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }}>
                    <div style={responsiveCardStyle}>
                        ...
                        {loadingApplications && <p style={styles.small}>Loading applications...</p>}
                                ...
                                <div key={application.applicationId} style={styles.listingCard}>
                                    <div style={{ display: "grid", gap: 10 }}>
                                        ...
                                        <div style={responsiveActions}>
                                            <button
                                                type="button"
                                                style={responsivePrimaryButton}
                                                onClick={() => updateApplication(application.applicationId)}
                                            >
                                                Update Application
                                            </button>
                                            ...
                                            <button
                                                type="button"
                                                style={responsiveButton}
                                                onClick={() => navigate(`/student-profile/${application.studentUserId}`)}
                                            >
                                                View Profile
                                            </button>
                                        </div>
